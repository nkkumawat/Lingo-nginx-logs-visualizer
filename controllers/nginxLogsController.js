...
module.exports = {
  getNginxLogs: (req, res, next) => {
    var fileName =
      req.cookies[constants.APP_NAME + "_file"] || constants.DEFAULT_FILE;
    var filter = {
      fileName: fileName,
    };
    logParaserHelper
      .getRequestCodeAndCount({ fileName: fileName })
      .then((dataRequest) => {
        logParaserHelper
          .getStatusRangeCountAndLink({ statusRange: "4", filter: filter })
          .then((data404) => {
            logParaserHelper
              .getStatusRangeCountAndLink({ statusRange: "5", filter: filter })
              .then((data502) => {
                logParaserHelper
                  .getMostRequestedUrls({ fileName: fileName })
                  .then((dataMost) => {
                    logParaserHelper
                      .getMostIpsCount({ fileName: fileName })
                      .then((dataIps) => {
  ...
  getNginxStatusLogs: (req, res, next) => {
    ...
    logParaserHelper
      .getStatusRangeCountAndLink({ statusRange: "2", filter: filter })
      .then((data2xx) => {
        logParaserHelper
          .getStatusRangeCountAndLink({ statusRange: "3", filter: filter })
          .then((data3xx) => {
            logParaserHelper
              .getStatusRangeCountAndLink({ statusRange: "4", filter: filter })
              .then((data4xx) => {
                logParaserHelper
                  .getStatusRangeCountAndLink({
                    statusRange: "5",
                    filter: filter,
                  })
                  .then((data5xx) => {
